...
export const projects: Project[] = [
  ...
  },
  {
    ...
    shortDescription:
      "Implementation and analysis of approximation algorithms for protein folding in the three-dimensional Hydrophobic-Polar (HP) model.",
    fullDescription:
      "A comprehensive bioinformatics project implementing and evaluating the Hart-Istrail 3/8-approximation algorithm and Ant Colony Optimization for the NP-complete 3D protein folding problem.",
    abstract:
        ...
        content: {
          ...
          subsections: [
              ...
              images: [
                {
                  ...
                  caption:
                    "A 2D HP lattice showing hydrophobic residues (red) and polar residues (blue). The protein sequence is folded as a self-avoiding walk on the grid.",
                  width: "60%",
                },
              ],
            },
            {
              ...
              content:
                "The energy of a conformation is determined by counting hydrophobic-hydrophobic contacts between non-adjacent residues in the sequence. Each H-H contact between residues that are neighbors on the grid but not consecutive in the sequence contributes -1 to the total energy.",
              math: [
                {
                  id: "energy-function",
                  latex:
                    "\\text{Score}(F) = -\\sum_{\\substack{i,j \\\\ \\text{adjacent on grid}}} \\delta(i, j)",
                  description:
                    "Energy function where δ(i,j) = 1 if both residues i and j are hydrophobic and non-consecutive in sequence",
                  displayMode: true,
                },
              ],
              images: [
                {
                  ...
                  caption:
                    "Scoring example showing four H-H contacts (orange lines) between non-consecutive hydrophobic residues, resulting in a score of -4.",
                  width: "60%",
                },
              ],
            },
          ...
          subsections: [
            {
              ...
              content:
                "The algorithm begins by decomposing the protein sequence into alternating runs of polar (0s) and hydrophobic (1s) residues. This decomposition respects a crucial parity constraint: in a 3D lattice, two residues can only be topological neighbors if their sequence positions differ by an odd number. This constraint fundamentally limits which hydrophobic residues can form contacts.",
              math: [
                {
                  id: "sequence-decomposition",
                  latex: "s = Z_0 I_1 Z_1 I_2 Z_2 \\cdots I_k Z_k",
                  description:
                    "Sequence decomposition where Z_i are runs of 0s (polar) and I_i are runs of 1s (hydrophobic)",
                  displayMode: true,
                },
              ],
            },
            ...
            },
            {
              ...
              content:
                'Each block is arranged in a "normal form" where hydrophobic residues (1s) are placed along a straight line (the "face" of the block), and polar zero-runs are folded as loops on one side. When multiple blocks are combined into superblocks, their faces align along the same line, creating a systematic structure where every second residue pair down the middle can potentially score.',
              images: [
                {
                  ...
                  caption:
                    "A 2D fold showing the selected folding point (green line). Blue boxes show y-blocks, red boxes show x-blocks, and black boxes show z-blocks. Orange lines indicate potential scoring positions.",
                  width: "70%",
                },
              ],
            },
            {
              ...
              content:
                "The 3D algorithm extends the 2D approach by repeatedly applying the 2D folding algorithm in adjacent (x,y) planes. Each layer is constructed with a specific number J of scoring H residues, and planes are folded face-to-face to maximize inter-layer contacts. The algorithm calculates the optimal layer size and number of layers to ensure systematic coverage.",
              math: [
                {
                  id: "layer-calculation",
                  latex:
                    "J = \\left\\lfloor \\frac{L - 2K + 1}{K} \\right\\rfloor, \\quad K = \\lfloor \\sqrt{\\min(N_x, N_y)} \\rfloor",
                  description:
                    "Calculation of layer size J and number of layers K, where L is sequence length and N_x, N_y are counts of x-type and y-type blocks",
                  displayMode: true,
                },
              ],
              images: [
                {
                  ...
                  caption:
                    "An ideal 3D fold showing multiple layers with face-to-face alignment. This structure maximizes contacts between layers.",
                  width: "65%",
                },
              ],
            },
              ...
              images: [
                {
                  ...
                  caption:
                    "A 3D fold of sequence S2-8 with J=2. The bottom two layers each contain exactly 2 scoring H residues, while the top layer handles remaining residues using the 2D algorithm.",
                  width: "65%",
                },
              ],
            },
            {
              ...
              content:
                "The algorithm achieves its 3/8-approximation by ensuring that each hydrophobic residue in the constructed fold can potentially form contacts in three directions: within its 2D plane, with the plane above, and with the plane below. The theoretical maximum number of contacts is bounded by the even-odd residue distribution.",
              math: [
                {
                  id: "contact-bound",
                  latex: "C_{3D}(S) = 4\\bigl(\\min\\{O(S), E(S)\\}\\bigr) + 2",
                  description:
                    "Upper bound on achievable contacts where O(S) and E(S) are odd and even positioned hydrophobic residues",
                  displayMode: true,
                },
                {
                  id: "approximation-formula",
                  latex:
                    "\\text{Contacts Achieved} \\geq 3 \\left( \\frac{O(S)}{2} - o(\\sqrt{O(S)}) \\right)",
                  description:
                    "Lower bound on contacts achieved by the algorithm, leading to the 3/8 approximation ratio",
                  displayMode: true,
                },
              ],
            },
          ...
          subsections: [
            {
              ...
              content:
                "In nature, ants initially explore randomly, but when they find food, they return to the nest while depositing pheromones. Shorter paths accumulate more pheromone (due to faster round-trips), attracting more ants and creating positive feedback. Over time, the colony converges on optimal routes. This process combines exploitation of good solutions (following strong pheromone trails) with exploration of new possibilities (random choices when pheromone levels are low).",
            },
            {
              id: "aco-3d-hp-adaptation",
              title: "Adapting ACO to the 3D HP Model",
              content:
                "For protein folding, each artificial ant constructs a conformation by sequentially choosing folding directions from {S, L, R, U, D} (straight/forward, left, right, up, down). These directions are relative to the current orientation, making the representation compact and intuitive. The algorithm maintains pheromone values τ_{i,d} for each position i and direction d, representing the collective wisdom of the ant colony.",
              math: [
                {
                  id: "aco-probability",
                  latex:
                    "p_{i,d} = \\frac{[\\tau_{i,d}]^\\alpha \\cdot [\\eta_{i,d}]^\\beta}{\\sum_{e \\in \\{S,L,R,U,D\\}} [\\tau_{i,e}]^\\alpha \\cdot [\\eta_{i,e}]^\\beta}",
                  description:
                    "Probability of choosing direction d at position i, where α weights pheromone influence and β weights heuristic influence",
                  displayMode: true,
                },
              ],
            },
            {
              ...
              content:
                "The heuristic function η_{i,d} captures immediate reward by evaluating potential hydrophobic contacts. It uses a Boltzmann distribution to convert energy differences into probabilities, where γ acts as an inverse temperature parameter controlling the greediness of the search.",
              math: [
                {
                  id: "heuristic-function",
                  latex: "\\eta_{i,d} = e^{-\\gamma \\cdot h_{i,d}}",
                  description:
                    "Heuristic function where h_{i,d} is the number of H-H contacts achieved by placing residue i in direction d",
                  displayMode: true,
                },
              ],
            },
            {
              ...
              content:
                "Construction begins at a randomly chosen position in the sequence, with the first residue placed at (0,0,0). The algorithm alternates between folding left and right segments based on the ratio of unfolded residues, preventing one side from consuming too much space. When collisions occur, the ant performs partial backtracking (typically rolling back 50% of the current fold) and retries from an earlier valid state.",
            },
            {
              id: "pheromone-dynamics",
              title: "Pheromone Update and Learning Mechanism",
              content:
                "After all ants complete construction, pheromone trails undergo two processes: evaporation and reinforcement. Evaporation prevents premature convergence by reducing all pheromone values by factor ρ. High-quality conformations then deposit new pheromone proportional to their energy score, creating stronger trails for successful strategies.",
              math: [
                {
                  id: "pheromone-evaporation",
                  latex: "\\tau_{i,d} \\leftarrow \\rho \\cdot \\tau_{i,d}",
                  description:
                    "Pheromone evaporation where 0 < ρ ≤ 1 is the persistence parameter",
                  displayMode: false,
                },
                {
                  id: "pheromone-deposit",
                  latex:
                    "\\tau_{i,d} \\leftarrow \\tau_{i,d} + \\Delta_{i,d,c}",
                  description:
                    "Pheromone reinforcement where Δ_{i,d,c} is proportional to conformation c's quality",
                  displayMode: false,
                },
              ],
            },
              ...
              images: [
                {
                  ...
                  caption:
                    "Example fold generated by the Ant Colony Optimization algorithm, showing a compact structure with multiple H-H contacts. Notice the creative folding pattern that differs significantly from the systematic layering of the Hart-Istrail approach.",
                  width: "60%",
                },
              ],
            },
              ...
              images: [
                {
                  ...
                  caption:
                    "Our 3D coordinate system: north/south along z-axis, east/west along x-axis, up/down along y-axis. This standard provides consistent spatial reference for all folding operations.",
                  width: "50%",
                },
              ],
            },
              ...
              images: [
                {
                  ...
                  caption:
                    "Initial state: forward direction (red arrow) pointing north, upward direction (green arrow) pointing up, with rotation = 0°.",
                  width: "45%",
                },
                {
                  ...
                  caption:
                    "After {forward, up, up}: forward direction rotated, upward direction now pointing down, rotation = 180°.",
                  width: "45%",
                },
              ],
            },
              ...
              images: [
                {
                  ...
                  caption:
                    "Complete visualization interface showing a 3D protein fold with ImGui control panel. Users can inspect individual residues, toggle visual elements, load/save configurations, and navigate the 3D space in real-time.",
                  width: "75%",
                },
              ],
            },
        ...
        content: {
          ...
          subsections: [
            ...
            },
            {
              ...
              content:
                "The Ant Colony Optimization consistently outperformed our Hart-Istrail implementation across all test sequences, achieving approximately 48% of optimal scores compared to 34%. This represents a significant improvement in solution quality. The ACO approach requires substantially more computation time (26.2 seconds vs. <0.1 seconds), but its ability to explore diverse conformational states through guided stochastic search leads to superior energy minimization.",
            },
            {
              id: "approximation-analysis",
              title:
                "Approximation Ratio Analysis and Implementation Challenges",
              content:
                "Our Hart-Istrail implementation did not achieve the theoretical 3/8-approximation ratio. Several factors contribute to this gap: incomplete handling of edge cases in block boundary processing, the relatively short sequence lengths (48 residues) where boundary effects dominate, and implementation complexity of the full algorithm specification. The approximation guarantee primarily holds for longer sequences where the systematic layering approach can achieve its theoretical performance.",
            },
            {
          ...
          customVisualizations: [
            {
              ...
              data: {
                ...
                caption:
                  "Comparison of energy scores achieved by ACO and Hart-Istrail algorithms on benchmark protein sequences. Lower (more negative) scores indicate better folding quality. ACO consistently outperforms Hart-Istrail, achieving nearly 50% of optimal performance compared to Hart-Istrail's 34%.",
              },
            },
            {
